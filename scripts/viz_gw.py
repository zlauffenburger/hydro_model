...
sim_path = '/home/zachary/workspace/pest/pest_dawuap_2016-2017/groundwater.json'
...
for node in nodes:
    ...
    gw = pd.DataFrame(date_list, columns=['date', 'flow'])
    gw = gw.rename(columns={'flow': id})
...
# df.to_csv('/home/zachary/Maps_Figures/gw_2007-2017.csv')
#
# df2 = pd.read_csv('/home/zachary/Maps_Figures/gw_2007-2017.csv')
df2 = df
# df2.set_index('date', inplace=True)
# df2.index = pd.to_datetime(df2.index)
# df2.drop(['242', '248'], axis=1, inplace=True)
...
plt.xticks(size=40)
plt.yticks(size=40)
plt.title('Groundwater', fontsize=50)
plt.legend(prop={'size': 20})
plt.xlabel('Date', fontsize=40)
plt.ylabel('Groundwater (mm)', fontsize=40)
